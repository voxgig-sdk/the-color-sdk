...
class TestSchemeDirect:

    def test_should_direct_list_scheme(self):
        setup = _scheme_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-scheme", "live" if setup["live"] else "unit")
        if _skip:
...
def _scheme_direct_setup(mockres):
