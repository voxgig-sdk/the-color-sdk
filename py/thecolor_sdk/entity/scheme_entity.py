...
from thecolor_sdk.thecolor_types import (
    Scheme,
    SchemeListMatch,
)
...
class SchemeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SchemeEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Scheme]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Scheme().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
