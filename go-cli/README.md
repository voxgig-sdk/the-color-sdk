...
boru-driven command-line client **and** interactive REPL for the TheColor
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/the-color-cli)
make build
...
./the-color-cli load 1 idn            # {id:1} shorthand
./the-color-cli load '{id:1}' idn       # explicit match map

# 5. Override the API base URL for a single call
THE_COLOR_BASE=https://api.example.com ./the-color-cli load 1 idn

# 6. No arguments -> interactive REPL
./the-color-cli
the-color> load 1 idn
the-color> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/the-color-cli
   ```
...
```sh
./the-color-cli load 1 idn          # scalar shorthand for {id:1}
./the-color-cli load '{id:1}' idn     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `the-color>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `idn`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/the-color-cli`. |
...
(`./the-color-cli <expr>`) and interactively (the REPL), and expressions compose the
