// Command TheColor-cli is a CLI + REPL for the TheColor SDK,
...
const prompt = "the-color"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "idn scheme"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      THE_COLOR_APIKEY (api key), THE_COLOR_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: THE_COLOR_APIKEY carries the API key and
	// THE_COLOR_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("THE_COLOR_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("THE_COLOR_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewTheColorSDK(opts)
