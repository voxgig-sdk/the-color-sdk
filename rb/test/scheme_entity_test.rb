...
class SchemeEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = TheColorSDK.test(seed, nil)
    seen = base.Scheme(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Scheme(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set THE_COLOR_TEST_SCHEME_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def scheme_basic_setup(extra)
  ...
  entid_env_raw = ENV["THE_COLOR_TEST_SCHEME_ENTID"]
