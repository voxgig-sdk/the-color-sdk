...
class SchemeDirectTest < Minitest::Test
  def test_direct_list_scheme
    setup = scheme_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-scheme", setup[:live] ? "live" : "unit")
    if _should_skip
...
def scheme_direct_setup(mockres)
  ...
  if live
    ...
    client = TheColorSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
