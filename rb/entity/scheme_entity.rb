...
class SchemeEntity
  ...
  def make
    opts = @_entopts.dup
    SchemeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = TheColorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Scheme, Hash] the current Scheme data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = TheColorHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Scheme fields)
  def match_get
  ...
  #
  # @param reqmatch [SchemeListMatch, Hash, nil] match filter (any subset of
  #   Scheme fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Scheme>, Array] the matching Scheme items; raises TheColorError on failure
