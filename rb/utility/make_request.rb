...
module TheColorUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = TheColorResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = TheColorResponse.new(fetched)
    else
