...
The Ruby SDK for the TheColor API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Idn` — with named operations (`list`/`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = TheColorSDK.test(testopts, sdkopts)
```
...
raise a `TheColorError` (a `StandardError` subclass), so wrap
...
```
rb/
├── TheColor_sdk.rb       -- Main SDK module
...
The main module (`TheColor_sdk`) exports the SDK class
