...
class FtCtrl extends TheColorControl
{
...
class FtClient
{
    ...
    private TheColorUtility $utility;

    public function __construct(TheColorUtility $utility, array $options)
    {
    ...
    public function get_utility(): TheColorUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (TheColorContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public TheColorContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (TheColorContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = TheColorConfig::make_feature($name);
    ...
    private static function buildUrl(TheColorSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof TheColorError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new TheColorResponse($fetched);
        }
    ...
    private function fail(TheColorContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (TheColorContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
