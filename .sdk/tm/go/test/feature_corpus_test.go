...
//
// NewTheColorSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.TheColorSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewTheColorSDK(opts)
}
...
func fcCandidates(client *sdk.TheColorSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.TheColorSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.TheColorSDK, name string) any {
