...
import (
	...
	omni "github.com/voxgig-sdk/the-color-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.TheColorError); is {
				out := map[string]any{
					"name":    "TheColorError",
...
func omniCtx(arg any, client *sdk.TheColorSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.TheColorSDK); is {
			client = live
