...
// TheColor SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class TheColorMakeError
{
    public static function call(?TheColorContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new TheColorContext([], null);
        }
        $op = $ctx->op ?? new TheColorOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new TheColorResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof TheColorError) ? $err->msg : (string)$err;
        $msg = "TheColorSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new TheColorError('', $msg, $ctx);
        ...
        if ($err instanceof TheColorError) {
