...
class SchemeEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new SchemeEntity($this->_client, $opts);
    }

    /**
     * @param Scheme|array $args Scheme data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = TheColorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Scheme|array The current Scheme data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Scheme fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = TheColorHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Scheme fields).
     */
    ...
    /**
     * List Scheme items matching the given filter.
     *
     * @param SchemeListMatch|array|null $reqmatch Match filter (any subset
     *   of Scheme fields) as an assoc-array; SchemeListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Scheme[]|array A list of Scheme items as assoc-arrays at
     *   the SDK boundary; throws TheColorError on failure (item-5 convention).
     */
