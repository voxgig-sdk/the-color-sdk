...
class TheColorNetsimFeature extends TheColorBaseFeature
{
    ...
    private function _simulate(TheColorContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(TheColorContext $ctx, array $applied): void
    {
        $client = $this->client;
