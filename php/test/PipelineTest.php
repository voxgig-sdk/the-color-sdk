...
{
    private static function utility(): TheColorUtility
    {
        return new TheColorUtility();
    }
    ...
    private static function ctx(array $over = []): TheColorContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new TheColorContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new TheColorOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof TheColorError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new TheColorOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = TheColorMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new TheColorOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = TheColorMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new TheColorOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = TheColorMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TheColorSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = TheColorMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new TheColorResponse([]),
            'result' => new TheColorResult([]),
        ]);
        [, $err] = TheColorMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new TheColorSpec([]),
            'response' => null,
            'result' => new TheColorResult([]),
        ]);
        [, $err] = TheColorMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new TheColorSpec(['step' => 's']),
            'response' => new TheColorResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new TheColorResult([]),
        ]);
        [, $err] = TheColorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new TheColorSpec(['step' => 's']),
            'response' => new TheColorResponse(self::resp(200, ['v' => 1])),
            'result' => new TheColorResult([]),
        ]);
        [, $err] = TheColorMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new TheColorSpec(['step' => 's']),
            'response' => new TheColorResponse(self::resp(200, ['v' => 2])),
            'result' => new TheColorResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        TheColorMakeResponse::call($ctx);
    ...
    {
        $preset = new TheColorResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new TheColorSpec([]),
            'response' => new TheColorResponse([]),
            'result' => new TheColorResult([]),
        ...
        [$got, $err] = TheColorMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new TheColorResult([])]);
        [, $err] = TheColorMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new TheColorSpec([]), 'result' => null]);
        [, $err] = TheColorMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TheColorSpec(['step' => 's']),
            'result' => new TheColorResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new TheColorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TheColorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new TheColorSpec(['step' => 's']),
            'result' => new TheColorResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new TheColorOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = TheColorMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new TheColorResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new TheColorSpec([]),
            'result' => new TheColorResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = TheColorMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TheColorMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new TheColorError('boom', 'boom');
        $utility->fetcher = function (TheColorContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TheColorSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = TheColorMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (TheColorContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = TheColorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(TheColorResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (TheColorContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new TheColorSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = TheColorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new TheColorResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new TheColorSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = TheColorMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = TheColorMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = TheColorMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new TheColorResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, TheColorDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new TheColorResult(['ok' => false])]);
        $this->expectException(TheColorError::class);
        TheColorDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new TheColorResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', TheColorMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        TheColorMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new TheColorBaseFeature();
        $b = new TheColorBaseFeature();
        TheColorFeatureAdd::call($ctx, $a);
        TheColorFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): TheColorBaseFeature
    {
        $f = new TheColorBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        TheColorFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): TheColorContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new TheColorSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = TheColorPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new TheColorResponse(['headers' => null]),
            'result' => new TheColorResult([]),
        ]);
        TheColorResultHeaders::call($ctx);
