...
class TheColorSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = TheColorHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = TheColorHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!TheColorFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, TheColorFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return TheColorUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = TheColorHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = TheColorHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = TheColorHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new TheColorError($op . "_allow",
                "TheColorSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = TheColorHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new TheColorError("graphql_error",
                "TheColorSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Scheme($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_scheme === null) {
                $this->_scheme = new SchemeEntity($this, null);
            }
            return $this->_scheme;
        }
        return new SchemeEntity($this, $data);
    }
