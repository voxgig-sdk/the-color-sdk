...
//     (\`new TheColorSDK(...)\` / \`TheColorSDK.test(...)\` become
//     \`TheColorSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"idn":{"test01":{"id":"test01"}},"scheme":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new TheColorSDK(...)\` and any
// \`TheColorSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+TheColorSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/TheColorSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
