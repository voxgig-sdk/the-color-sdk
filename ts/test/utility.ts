...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.THECOLOR_TEST_LIVE ||
    'TRUE' === process.env.THECOLOR_TEST_OVERRIDE
  ) {
  ...
  m.THECOLOR_TEST_EXPLAIN = process.env.THECOLOR_TEST_EXPLAIN || m.THECOLOR_TEST_EXPLAIN
