...
The Lua SDK for the TheColor API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Idn(nil):load({ id = "example_id" }, nil)
...
local result, err = client:TheColor(nil):load(
  { id = "test01" }, nil
)
