...
local SchemeEntity = {}
SchemeEntity.__index = SchemeEntity


function SchemeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SchemeEntity)
  self._name = "scheme"
  self._client = client
...
function SchemeEntity:get_name()
  return self._name
end


function SchemeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SchemeEntity.new(self._client, opts)
end
...
function SchemeEntity:mark_deleted()
  self._deleted = true
end


function SchemeEntity:deleted()
  return true == self._deleted
end


function SchemeEntity:data_set(args)
  if args ~= nil then
...
function SchemeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SchemeEntity:match_set(args)
  if args ~= nil then
...
function SchemeEntity:match_get()
...
function SchemeEntity:stream(action, args, callopts)
...
function SchemeEntity:_run_op(ctx, post_done)
  local utility = self._utility
