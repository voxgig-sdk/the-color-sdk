...
[MCP](https://modelcontextprotocol.io) server exposing the TheColor SDK as
two agent tools — `the-color_list` and `the-color_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/the-color-mcp)
make build

# 2. Provide credentials via the environment
export THE_COLOR_APIKEY=sk_live_xxx
...
```jsonc
// the-color_list: first page of records
{ "entity": "scheme" }
{ "entity": "scheme", "query": { } }

// the-color_load: one record by id
{ "entity": "idn", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/the-color-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user the-color \
     -- "$PWD"/dist/*/the-color-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `the-color_list` and `the-color_load` tools now appear
   in new sessions. Ask the agent to *"list scheme using the-color"*
   and it calls `the-color_list` with `{"entity":"scheme"}`.
...
```sh
export THE_COLOR_APIKEY=sk_live_xxx            # API key
export THE_COLOR_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `the-color_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `the-color_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `THE_COLOR_APIKEY` | API key sent with every request. |
| `THE_COLOR_BASE` | Optional override of the API base URL. |
